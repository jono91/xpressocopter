...
uint8_t getEstimatedAltitude(){
    ...
    sonarSmooth = applyMedFilter(&SonarFilter,sonarAlt) * halfvz * 2;
#else
    sonarSmooth = sonarAlt;
#endif
    ...
    if (!f.ARMED) {
        ...
        sonarOffset += sonarSmooth;
    }
    sonarSmooth -= sonarOffset>>3;
    if(EstAlt < 400)
        EstAlt = (EstAlt * 2 + sonarSmooth * 6) >> 3;// complimentary filter to include sonar data
    else
