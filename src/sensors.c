...
{
    if(i2c_read(PX4FLOW_ADDRESS,PX4FLOW_FRAME_CNT_ADDR,2) == I2CSTATE_ACK)
    {
...
{
    if(i2c_read(PX4FLOW_ADDRESS,PX4FLOW_VEL_ADDR,8) == I2CSTATE_ACK)
    {
        flow_comp_m_x = (I2CSlaveBuffer[1]<<8) | (I2CSlaveBuffer[0]);
        flow_comp_m_y = (I2CSlaveBuffer[3]<<8) | (I2CSlaveBuffer[2]);
        qual = I2CSlaveBuffer[4];
        sonar_timestamp = I2CSlaveBuffer[5];
        ground_distance = (I2CSlaveBuffer[7]<<8) | (I2CSlaveBuffer[6]);
    }
    else
        i2c_errors_count++;
}
...
void initSensors() {
    ...
    delayMs(100);
#ifdef FLOW
    probeFlowSensor(); //read from PX4Flow first so it does not ack packets destined for other sensors
#endif
    if (GYRO) Gyro_init();
    if (BARO) Baro_init();
    if (MAG) Mag_init();
